...
const ImportRunArgs = z.object({
  ...
  "skip-model": z
    .boolean()
    .default(false)
    .describe("Skip downloading model files"),
...
`,
  ...
  async run(args, ctx) {
    ...
    logStep(
      { step: "download", message: "Downloading run metadata..." },
      ctx.output,
    );
    ...
    if (!statusResult.success) {
      return error(
        "STATUS_NOT_FOUND",
        `Run not found or incomplete: ${runName}`,
    ...
    if (status.phase !== "completed") {
      return error(
        "NOT_COMPLETED",
        `Run not completed (phase: ${status.phase})`,
    ...
    await download(
      `${gcsPrefix}/artifacts/sample_predictions.json`,
      predictionsPath,
    );
    ...
    if (!args["skip-model"]) {
      logStep(
        { step: "model", message: "Downloading model files..." },
        ctx.output,
      );
    ...
    return success({
      ...
      metrics: {
        ...
        duration: status.duration_seconds
          ? `${((status.duration_seconds as number) / 60).toFixed(1)} min`
          : "unknown",
      },
